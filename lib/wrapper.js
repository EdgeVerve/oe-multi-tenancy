...
module.exports = function (app) {
  ...
  var newProperties = { properties: { upward: {
    type: 'Boolean',
    required: false,
    default: false
  },
...
function _newCreateOptionsFromRemotingContext(ctx) {
  ...
  if (!autoScope) {
    if (options && options.accessToken && options.accessToken.ctx) {
      options.ctx2 = Object.assign({}, options.accessToken.ctx);
    }
    return options;
  }
  ...
  options.ctx = Object.assign({}, options.accessToken.ctx);
  var autoscopeEmpty = true;
  for (var i = 0; i < autoScope.length; ++i) {
    if (options.accessToken && options.accessToken.ctx && options.accessToken.ctx[autoScope[i]]) {
      autoscopeEmpty = false;
      break;
    }
  }
  log.debug(options.ctx, 'Options found in request ', options.ctx);
  if (autoscopeEmpty && model.setOptions) {
    options = model.setOptions(ctx, this);
  }

  if (autoscopeEmpty && rootModel.setOptions) {
    options = rootModel.setOptions(ctx, this);
  }
  // set same callContext in request
  // if (ctx && ctx.req) {
  //    ctx.req.callContext = options;
  //  }
  return options;
}
