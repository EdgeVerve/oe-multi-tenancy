...
function _newCreateOptionsFromRemotingContext(ctx) {
  ...
  for (var i = 0; i < autoScope.length; ++i) {
    if (options.accessToken && options.accessToken.ctx) {
      options.ctx[autoScope[i]] = options.accessToken.ctx[autoScope[i]];
    }
  }
  log.debug(options.ctx, 'Options found in request ', options.ctx);
  if (_.isEmpty(options.ctx) && model.setOptions) {
    options = model.setOptions(ctx, this);
  }

  if (_.isEmpty(options.ctx) && rootModel.setOptions) {
...
function findByIdAndCreate(self, id, data, options, cb) {
  ...
  if (typeof opts.ignoreAutoScope === 'undefined') {
    opts.ignoreAutoScope = true;
    flagOptionsModified = true;
  }
  self.find(q, options, function (err, r) {
    if (flagOptionsModified) {
      delete opts.ignoreAutoScope;
    }
    if (err) {
